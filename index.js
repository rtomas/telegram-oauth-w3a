...
const TELEGRAM_APP_NAME = process.env.TELEGRAM_APP_NAME;
const TELEGRAM_REDIRECT_URI = process.env.TELEGRAM_REDIRECT_URI;
const TELEGRAM_APP_TOKEN = process.env.TELEGRAM_APP_TOKEN;
...
app.get("/telegram/login", async (req, res) => {
    ...
    fs.readFile(file, 'utf8', (err, data) => {
        ...
        const result = data.replace(/{{TELEGRAM_BOT_NAME}}/g, TELEGRAM_APP_NAME).replace(/{{TELEGRAM_BOT_CALLBACK}}/g, TELEGRAM_REDIRECT_URI);
...
app.get("/telegram/callback", async (req, res) => {
    // validate the user
    const token = TELEGRAM_APP_TOKEN;
